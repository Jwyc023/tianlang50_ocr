...
"""
单张截图识别程序
使用monitor_precision中的高精度识别方法识别指定的截图
"""
...
def multi_engine_recognize(img: np.ndarray, ocr_manager: OCRManager, is_grade: bool = False) -> Tuple[str, float]:
    ...
    print(f"使用 {len(engines)} 个OCR引擎，{len(processed_images)} 种预处理方案")
        ...
        for i, processed_img in enumerate(processed_images):
            try:
                ...
                if text.strip():
                    ...
                    print(f"  {engine} 方案{i+1}: '{text}' (置信度: {confidence:.2f})")
            except Exception as e:
                print(f"  {engine} 方案{i+1} 失败: {e}")
...
def recognize_single_image(image_path: str, roi_config_path: str = None, save_debug: bool = True) -> Dict:
    """
    识别单张截图
    ...
    results = {
        ...
        "ocr_engines": list(ocr_manager.engines.keys()),
        "roi_results": {},
        "full_image_results": {}
    }
    ...
    # 识别整个图片（如果没有ROI配置或作为补充）
    print(f"\n=== 识别整个图片 ===")
    
    # 数字识别
    print("识别数字内容:")
    txt_numbers, numbers_conf = multi_engine_recognize(img, ocr_manager, is_grade=False)
    
    # 字母识别
    print("识别字母内容:")
    txt_letters, letters_conf = multi_engine_recognize(img, ocr_manager, is_grade=True)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="单张截图识别程序")
